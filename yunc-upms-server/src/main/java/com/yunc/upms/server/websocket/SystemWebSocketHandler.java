...
public class SystemWebSocketHandler implements WebSocketHandler {
	...
	@Autowired
	private IUpmsUserService loginservice;
	...
	public void afterConnectionEstablished(WebSocketSession session)
		...
		UpmsUser username=loginservice.selectById(uid);
		if (userSocketSessionMap.get(uid) == null) {
			userSocketSessionMap.put(uid, session);
			Message msg = new Message();
			msg.setFrom(0L);//0表示上线消息
			msg.setText(username.getUsername());
	...
	public void handleTransportError(WebSocketSession session,
		...
		while (it.hasNext()) {
			...
			if (entry.getValue().getId().equals(session.getId())) {
				...
				UpmsUser username=loginservice.selectById(entry.getKey());
				Message msg = new Message();
				msg.setFrom(-2L);
				msg.setText(username.getUsername());
				this.broadcast(new TextMessage(new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").create().toJson(msg)));
