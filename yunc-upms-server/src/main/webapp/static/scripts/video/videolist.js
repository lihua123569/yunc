	...
	$("body").on("click",".setb-btn",function(){
		...
		function a(){
			$.ajax({
				type: 'POST',
				url:"/videoPic/json/delete?id="+_dataid,
				async: false,
				success:function(result){
					result = eval("("+result+")");
					if(result.responseCode =="0000"){
	...
	$("#add_video").click(function(){
		window.location.href="/videoPic/addVideo";
	});
	...
	function query(conf,dtd){
		...
		$.ajax({
			type: 'POST',
			url:"/videoPic/json/getVideoPicList",
			async: false,
			data:params,
			success:function(result){
				result = eval("("+result+")");
